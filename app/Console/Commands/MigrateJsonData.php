...
class MigrateJsonData extends Command
{
    ...
    private function migrateAdmins($sourceDir)
    {
        ...
        foreach ($adminsData as $adminData) {
            ...
            $existingAdmin = Admin::where('id', $adminData['id'])->first();
            
            if (!$existingAdmin) {
                Admin::create([
                    'id' => $adminData['id'],
                    'username' => $adminData['username'],
                    'password_hash' => $adminData['password_hash'], // Already hashed
                    'created_at' => $adminData['created_at'],
                ]);
                
                $this->info("Admin '{$adminData['username']}' migrated.");
            } else {
                $this->info("Admin '{$adminData['username']}' already exists, skipping.");
            }
        }
    }
}